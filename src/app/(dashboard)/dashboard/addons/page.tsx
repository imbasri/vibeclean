...
export default function AddonsPage() {
  ...
  const [selectedPurchase, setSelectedPurchase] = useState<AddonPurchase | null>(null);
  ...
  const fetchAddons = async () => {
    ...
    try {
      ...
      gooeyToast.error("Gagal memuat data add-on", { description: "Silakan coba lagi" });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handlePurchase = async () => {
    ...
    if (selectedProduct.type === "custom_domain" && !customDomain) {
      gooeyToast.error("Domain diperlukan", { description: "Masukkan nama domain yang diinginkan" });
      return;
    }
    ...
    try {
      const response = await fetch("/api/addons/purchase", {
        ...
        body: JSON.stringify({
          ...
          customDomain: selectedProduct.type === "custom_domain" ? customDomain : undefined,
        }),
      });
      ...
      if (data.success) {
        gooeyToast.success("Berhasil", { description: "Add-on berhasil dibeli!" });
        setShowConfirmDialog(false);
        setCustomDomain("");
        setSelectedProduct(null);
        fetchAddons(); // Refresh data
      } else {
        gooeyToast.error("Gagal", { description: data.error || "Gagal membeli add-on" });
      }
    } catch (error) {
      console.error("Error purchasing addon:", error);
      gooeyToast.error("Gagal", { description: "Terjadi kesalahan saat pembelian" });
    } finally {
  ...
  const handleVerify = async () => {
    ...
    setIsVerifying(true);
    try {
      const response = await fetch(`/api/addons/${selectedPurchase.id}/verify`, {
        method: "POST",
      });
      const data = await response.json();
      
      if (data.success) {
        gooeyToast.success("Domain Verified", { description: "Custom domain berhasil diverifikasi!" });
        fetchAddons();
        setShowDNSDialog(false);
      } else {
        gooeyToast.error("Verification Failed", { description: data.message || "Gagal verifikasi domain" });
      }
    } catch (error) {
      gooeyToast.error("Error", { description: "Gagal verifikasi domain" });
    } finally {
              ...
              <motion.div key={product.id} variants={itemVariants}>
                <Card className="h-full flex flex-col">
                  ...
                  <CardContent className="flex-grow">
                    {product.type === "whatsapp_quota" && product.quota && (
                      <div className="flex items-center gap-2 text-sm text-muted-foreground mb-4">
                        <Zap className="h-4 w-4" />
                        {product.quota} pesan WhatsApp
                      </div>
                    )}
                    {product.type === "custom_domain" && (
                    ...
                    <Card>
                      <CardHeader className="flex flex-row items-center justify-between">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <CardTitle>{product?.name || "Add-on"}</CardTitle>
                            <CardDescription>
                              {purchase.customDomain 
                                ? purchase.customDomain 
                      ...
                      <CardContent>
                        <div className="flex items-center gap-4 text-sm text-muted-foreground">
                          ...
                          {!purchase.customDomain && (
                            <div>
                              Usage: {purchase.whatsappUsed} / {purchase.whatsappQuota}
                            </div>
                          )}
                        </div>
                        {purchase.customDomain && (
                          <Button 
                            ...
                            size="sm" 
                            className="mt-4"
                            onClick={() => openDNSDialog(purchase)}
                          >
                      ...
                      <CardHeader className="flex flex-row items-center justify-between">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <CardTitle>{product?.name || "Add-on"}</CardTitle>
                            <CardDescription>
                              {purchase.customDomain || `${purchase.whatsappQuota} pesan`}
                            </CardDescription>
                          </div>
                        </div>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="bg-muted rounded-lg p-4 space-y-2">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium">{selectedProduct?.durationDays} hari</span>
              </div>
      ...
      <Dialog open={showDNSDialog} onOpenChange={setShowDNSDialog}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Ikuti langkah berikut untuk mengaktifkan custom domain Anda
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-6 py-4">
            ...
            <div className="bg-muted rounded-lg p-4">
              ...
              <p className="font-mono text-lg font-medium">{selectedPurchase?.customDomain}</p>
            </div>

            {/* DNS Instructions */}
            {!selectedPurchase?.customDomainVerified && (
              <div className="space-y-4">
                <h4 className="font-medium">DNS Records yang perlu di-setup:</h4>
                ...
                <div className="space-y-3">
                  ...
                  <div className="border rounded-lg p-4">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-2">
                          <code className="bg-muted px-2 py-1 rounded">76.76.21.21</code>
                          <Button 
                            variant="ghost" 
                            size="icon" 
                            className="h-6 w-6"
                  ...
                  <div className="border rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Badge variant="outline">Optional (www)</Badge>
                    </div>
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Type:</span>
                      ...
                      <div className="flex justify-between items-center">
                        <span className="text-muted-foreground">Value:</span>
                        <div className="flex items-center gap-2">
                          <code className="bg-muted px-2 py-1 rounded">cname.vercel-dns.com</code>
                          <Button 
                            variant="ghost" 
                            size="icon" 
                            className="h-6 w-6"
                ...
                <p className="text-sm text-muted-foreground">
                  Setelah setup DNS, perubahan dapat memakan waktu hingga 24 jam untuk propagate.
                </p>
              </div>
            )}
